...
function App() {
    ...
    if(tour.length < 1){
      return (
        <div className='flex flex-col justify-between items-center'>
          <h1 className='text-[4rem] font-bold'>No tours left</h1>
          <button onClick={() => setTour(data)} className='border-2 border-solid border-[#b4161b] text-[28px] font-bold bg-[#b4161b21] rounded-[10px] hover:bg-[red] hover:text-white transition-all duration-200ms px-16 py-4'>Refresh</button>
        </div>
      );
    }
  ...
  return (
    <div>
      <p className='w-[350px] mx-auto mt-6 mb-10 outline-dashed outline-4 outline-[#1c1c93] flex justify-center items-center py-2 rounded-[10px] text-[2rem] font-bold'>Plan With Ayush</p>
